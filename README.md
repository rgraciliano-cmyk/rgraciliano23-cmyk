# Olá, eu sou o Rafaell Graciliano! 👋

**Analista de Infraestrutura Júnior | Supervisor tecnico | Acadêmico de ADS (2/5)**

Profissional com 5 anos de bagagem técnica em sistemas críticos e tecnologia IP, atualmente focado em **Infraestrutura de TI e Segurança Cibernética**. Especializado em prover suporte N2 e gerenciar ambientes de alta disponibilidade, unindo o desenvolvimento de software (ADS) com a prática de virtualização e redes.

---

## 🧰 Tecnologias e Ferramentas
<p align="left">
  <img src="https://img.shields.io/badge/Proxmox-E74C3C?style=for-the-badge&logo=proxmox&logoColor=white" />
  <img src="https://img.shields.io/badge/OpenWrt-121511?style=for-the-badge&logo=openwrt&logoColor=white" />
  <img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black" />
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/Oracle-F80000?style=for-the-badge&logo=oracle&logoColor=white" />
  <img src="https://img.shields.io/badge/Zabbix-CC0000?style=for-the-badge&logo=zabbix&logoColor=white" />
  <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white" />
</p>

## 🚀 Projetos em Destaque
* **[Automação de Backup Híbrido](https://github.com/rgraciliano23-cmyk/infra-backup-automation):** Script em Python para backup automatizado via SSH/SFTP (Porta 22) com gestão de retenção e notificações SMTP.
* **[Hardening de Firewall OpenWrt](https://github.com/rgraciliano23-cmyk/homelab):** Implementação de regras de mitigação de ataques DDoS e Rate Limiting em ambiente virtualizado.
* **[Monitoramento Zabbix]:** Dashboards para acompanhamento de saúde de hardware (Dell PowerEdge) e serviços Cloud.

## 🛠️ Experiência & Atuação
- **Supervisão & Suporte N2:** Atuação como ponto de escalonamento técnico avançado para equipes de campo e interface direta com TI Corporativo.
- **Infraestrutura Híbrida:** Integração de servidores locais (Bare Metal) com instâncias em nuvem (OCI Always Free).
- **Segurança Eletrônica IP:** Domínio de plataformas **MONI, iVMS e ISS**, com foco em CFTV avançado e controle de acesso.
- **Metodologia:** Utilizo Inteligência Artificial (Gemini) como copiloto para otimização de scripts, documentação técnica e aceleração de diagnósticos.

## 🚀 Meu Lab de Estudo (Homelab)
Simulo ambientes corporativos reais para aplicar os conceitos acadêmicos de ADS:
- **Hipervisor:** Proxmox VE 7.x (Hardware Dell PowerEdge)
- **Networking:** Firewall virtualizado com VPN WireGuard e segmentação de VLANs.
- **Automação:** Scripts Bash e Python para tarefas repetitivas e deploys.

## 📊 Estatísticas do GitHub
<p align="left">
  <img src="https://github-readme-stats.vercel.app/api?username=rgraciliano23-cmyk&show_icons=true&theme=tokyonight&include_all_commits=true" alt="Estatísticas de Commits" />
  <br>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=rgraciliano23-cmyk&layout=compact&theme=tokyonight" alt="Linguagens mais usadas" />
</p>

---

## 📫 Conecte-se comigo
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/rafaell-graciliano-25134a1bb)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/rgraciliano23-cmyk)
